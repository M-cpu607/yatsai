...
function SupabaseVideoCard({ data, onPlay }) {
  ...
  return (
    <div className="relative h-screen snap-start flex flex-col"
      ...
      {/* Infos en bas */}
      <div className="absolute bottom-24 left-4 right-4 fade-in">
        <div className="flex items-center gap-2 mb-2">
          ...
          <div className="flex-1 min-w-0">
            ...
            {data.position && (
              <div className="text-xs truncate" style={{ color: C.textDim }}>
                {sport?.icon || '🏆'} {sport?.label || data.sport} · {data.position}
              </div>
            )}
          </div>
        </div>
    ...
    return (
      <div className="min-h-screen flex flex-col items-center justify-center px-6"
        ...
        <p className="text-sm text-center" style={{ color: C.textDim }}>
          Sois le premier à publier !<br />
          Clique sur le bouton + en bas.
        </p>
      </div>
    );
  }
    ...
    <div className="min-h-screen pb-24" style={{ backgroundColor: C.bg }}>
      ...
      <form onSubmit={handlePublish} className="px-5 space-y-4">
        ...
        <button type="submit" disabled={loading}
          ...
          {loading ? 'Publication...' : 'Publier ma vidéo'}
        </button>
      </form>
    </div>
  );
...
function ProfileView({ onSwitchToRecruiter, userProfile, userEmail, onLogout }) {
  return (
    <div className="pt-12 pb-32 px-4" style={{ height: '100dvh', backgroundColor: C.bg }}>
      <div className="flex flex-col items-center mb-6 fade-in">
        <div className="w-24 h-24 rounded-full flex items-center justify-center text-3xl font-bold mb-3"
          style={{ backgroundColor: C.surface, color: C.gold, border: `2px solid ${C.gold}` }}>
          {userProfile?.full_name?.charAt(0)?.toUpperCase() || userEmail?.charAt(0)?.toUpperCase() || '?'}
        </div>
        <div className="flex items-center gap-2 mb-1">
<h2 className="text-xl font-extrabold" style={{ color: C.text }}>{userProfile?.full_name || 'Nouvel utilisateur'}</h2>
          <BadgeCheck size={18} fill={C.gold} stroke={C.bg} strokeWidth={2.5} />
        </div>
        <span className="text-sm" style={{ color: C.textDim }}>{userEmail || ''}</span>
      ...
      <div className="flex flex-col gap-2">
        <button onClick={onLogout}
          className="w-full py-3 rounded-xl flex items-center justify-center gap-2 text-sm font-semibold mt-4"
...
// ─── AUTHENTIFICATION SUPABASE ─────────────────────────────────
  ...
  const renderScreen = () => {
    ...
    switch (tab) {
      ...
      case 'messages': return <MessagesView videos={filtered.videos} onSelectAthlete={setSelectedAthlete} onOpenChat={setActiveChat} />;
      case 'profile':   return <ProfileView onSwitchToRecruiter={() => switchMode('recruiter')} userProfile={userProfile} userEmail={session?.user?.email} onLogout={async () => { await supabase.auth.signOut(); }} />;
      default: return null;
    }
  };
    ...
    return (
      <div className="min-h-screen flex items-center justify-center" style={{ backgroundColor: '#080F20' }}>
        <div className="text-3xl font-extrabold" style={{ color: '#FFFFFF' }}>
          Sco<span style={{ color: '#FFB800' }}>lympia</span>
        </div>
      </div>
    );
  }
